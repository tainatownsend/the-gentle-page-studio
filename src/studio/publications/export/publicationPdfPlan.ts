    ...
    if (page.kind === 'content' && pageNumber !== undefined) {
      page.blocks.forEach((block, index) => {
        ...
        if (block.type === 'rating-field' && rect) {
          const min = Math.min(block.min, block.max)
          const max = Math.max(block.min, block.max)
          const options = Array.from({ length: max - min + 1 }, (_, optionIndex) =>
            String(min + optionIndex),
          )
