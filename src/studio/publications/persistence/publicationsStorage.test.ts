...
describe('publicationsStorage', () => {
  ...
  it('loads a valid version 3 workspace', () => {
    localStorage.setItem(
      PUBLICATIONS_STORAGE_KEY,
      JSON.stringify({ version: 3, publications: [publication] }),
    )
  ...
  it.each([
    ...
    [
      ...
      JSON.stringify({
        version: 3,
        publications: [
          { ...publication, content: { blocks: [{ id: '', type: 'paragraph', text: 'Invalid' }] } },
        ],
      }),
    ],
    [
      ...
      JSON.stringify({
        version: 3,
        publications: [
          { ...publication, documentSettings: { ...publication.documentSettings, orientation: 'landscape' } },
        ],
      }),
    ],
