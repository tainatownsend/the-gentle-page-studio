...
export function auditPublicationVisualQuality(
  ...
  for (const component of inferPublicationCompoundComponents(flattenedBlocks)) {
    ...
    if (options.capacityUnits !== undefined && options.estimateUnits) {
      ...
      // A compound tool that is intrinsically larger than one page is allowed to become an
      // intentional multi-page tool. Visual QA only flags avoidable fragmentation.
